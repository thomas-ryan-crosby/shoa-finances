...
function createRevenueOverview() {
    const container = document.getElementById('revenueOverview');
    if (!container || !budgetData || !budgetData.income) return;
    
    const incomeItems = Object.entries(budgetData.income || {})
        .filter(([name, amount]) => amount > 0)
        .sort((a, b) => b[1] - a[1]);
    
    const totalIncome = Object.values(budgetData.income || {}).reduce((a, b) => a + b, 0);
